...
if (isset($_COOKIE['paspost_id'])) {
    ...
    $sql = 'SELECT u.nickname,u.icon,u.id,u.intro,u.hide  FROM follow as f JOIN pp_user as u ON f.follower_id=u.id WHERE following_id=? ORDER BY created_at DESC';
    $stmt = $dbh->prepare($sql);
    $stmt->execute(array($user_id));
...
<head>
    ...
    <title><?= $rec4['nickname'] ?>のフォロワー -パスポスト</title>
</head>
