...
$icon = $_SESSION['icon'];
$id_letter = $_REQUEST['id'];
$id_letter_json = json_encode($id_letter);

$sql = "SELECT message,cur_age,pos_age,category FROM letter WHERE id=?";
$stmt = $dbh->prepare($sql);
$stmt->execute(array($id_letter));
...
<body>
            ...
            <div id="posting" class="form-wrapper">
                ...
                <?php if ($rec) { ?>
                    <div class="form-contents">
                        ...
                        <button id="letter-edit-btn">変更する</button>
                ...
                <div class="modal-bg close">
                    <div class="modal-confirm modal">
                        <p class="header">この手紙を削除する</p>
                        <p>本当に手紙を消してもいいですか？削除すると復活させることはできません</p>
                        <div>
                            <button class="modal-cancel">キャンセル</button>
                            <button class="letter-delete-btn">削除する</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        var letterid = JSON.parse('<?= $id_letter_json ?>');
        ...
        const button = document.querySelector('#letter-edit-btn');
                ...
                $.ajax({
                    ...
                    if (data == 1) {
                        ...
                        $.ajax({
                            ...
                            location.href = `./letter.php?id=${letterid}`;
                        });
                    } else {
                        var wordCheck = 0;
                        $.ajax({
        ...
        $('.letter-delete-btn').one('click', function() {
            $.ajax({
                type: "POST",
                url: "ajax_letter_delete.php",
                datatype: "json",
                data: {
                    "letterid": letterid,
                }
